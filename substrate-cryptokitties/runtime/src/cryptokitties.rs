...
decl_module! {
  pub struct Module<T: Trait> for enum Call where origin: T::Origin {
    fn create_random_cat(origin, name: Vec<u8>) -> Result {
      ...
      let random_seed = (<system::Module<T>>::random_seed(), &sender).using_encoded(<T as system::Trait>::Hashing::hash);
      let dna = random_seed;

      Self::add_cat(sender, name, dna)?;
    ...
    fn breed_cat(origin, name: Vec<u8>, index1: u32, index2: u32) -> Result{
      let sender = ensure_signed(origin)?;

      ensure!(index1 < Self::kitty_count(), "Cat 1 does not exist");
      ensure!(index2 < Self::kitty_count(), "Cat 2 does not exist");

      let dna1 = Self::kitty(index1).dna;
      let dna2 = Self::kitty(index2).dna;
      ...
      Self::add_cat(sender, name, final_dna)?;
...
impl<T: Trait> Module<T> {
  fn add_cat(sender: T::AccountId, name: Vec<u8>, dna: T::Hash) -> Result {
