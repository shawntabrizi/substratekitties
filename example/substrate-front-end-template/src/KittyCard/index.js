  ...
  return (
    <Card>
      ...
      <Card.Content>
        <Card.Header>
          <h4>
            <span className='with-limit'>Kitty Index: {kittyIndex}</span>
          </h4>
        </Card.Header>
        <Card.Meta>
          <span className='with-limit'>
            DNA: <b>{bnToHex(kitty.dna)}</b>
          </span>
          <span className='with-limit'>
            Owner: <b>{encodeAddress(kitty.owner)}</b>
          </span>
        </Card.Meta>
      </Card.Content>
      <Card.Content extra>
        {kitty.price ? `Price: ${kitty.price}` : 'Not for sale'}
      </Card.Content>
    </Card>
  );
}
